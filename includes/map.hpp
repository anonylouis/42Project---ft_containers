	...
	class map
	{
		public :
			...
			class map_iterator
			{
				public :
					...
					map_iterator(N *itr = 0) : _ptr(itr) {};
					map_iterator(map_iterator const& copy) : _ptr(copy._ptr) {};
					...
					map_iterator operator++(int) {map_iterator save(*this);++(*this);return save;};
					...
					friend bool operator== (map_iterator const& a, map_iterator const& b) {return a._ptr == b._ptr;};
					friend bool operator!= (map_iterator const& a, map_iterator const& b) {return a._ptr != b._ptr;};

					//CONVERSION TO CONST
					operator map_iterator<const value_type, N>() const {return map_iterator<const value_type, N>(_ptr);};
			...
			typedef	RBT_node<value_type, Key, Compare, Alloc> node;
			...
			//CONSTRUCTOR
			explicit map (const key_compare& comp = key_compare(), const allocator_type& alloc = allocator_type()) : _comp(comp), _alloc_pair(alloc), _alloc_node(allocator_node()),_base_tree(0), _size(0)
			{
			...
			template <class InputIterator>
  			map (InputIterator first, InputIterator last, const key_compare& comp = key_compare(), const allocator_type& alloc = allocator_type()) : _comp(comp), _alloc_pair(alloc), _alloc_node(allocator_node()), _base_tree(0), _size(0)
			{
				min_node = _alloc_node.allocate(1);
				_alloc_node.construct(min_node, (value_type *) 0);
				max_node = _alloc_node.allocate(1);
			...
			map (const map& x) : _comp(x._comp), _alloc_pair(x._alloc_pair), _alloc_node(x._alloc_node), _base_tree(0), _size(x._size)
			{
				min_node = _alloc_node.allocate(1);
				_alloc_node.construct(min_node, (value_type *) 0);
				max_node = _alloc_node.allocate(1);
				...
				while (first != last)
				{
					temp_value = _alloc_pair.allocate(1);
					_alloc_pair.construct(temp_value, *first);
			...
			map& operator= (const map& x)
			{
				...
				map::const_iterator x_first = x.begin();
				map::const_iterator x_last = x.end();
				...
				{
					temp_value = _alloc_pair.allocate(1);
					_alloc_pair.construct(temp_value, *x_first);
					temp_node = _alloc_node.allocate(1);
					_alloc_node.construct(temp_node, temp_value);
					++x_first;
			...
			{
				ft::pair<iterator,bool> to_find = this->insert(ft::make_pair(k, mapped_type()));
				return ((*(to_find.first)).second);
			};
			...
			{
				delete_extremum_node(_base_tree);
				node *temp = RBT_find(_base_tree, val.first, _comp);
				if (temp)
				{
					set_extremum_node(_base_tree, min_node, max_node);
					return (ft::make_pair(iterator(temp), false));
				}
				value_type	*temp_value = _alloc_pair.allocate(1);
				_alloc_pair.construct(temp_value, val);
			...
			iterator insert (iterator position, const value_type& val)
			{
				...
				if (position != this->end())
					RBT_insert_position(&_base_tree, (*position).first, temp_node, _comp);
				else
					RBT_insert(&_base_tree, temp_node, _comp);
				set_extremum_node(_base_tree, min_node, max_node);
				_size++;
  			...
  			void insert (InputIterator first, InputIterator last)
			{
				...
				while (first != last)
				{
					if (RBT_find(_base_tree, (*first).first, _comp) == 0)
					{
						value_type	*temp_value = _alloc_pair.allocate(1);
						_alloc_pair.construct(temp_value, *first);
						node *temp_node = _alloc_node.allocate(1);
			...
			{
				delete_extremum_node(_base_tree);
				node* to_delete = RBT_find(_base_tree, (*position).first, _comp);
				if (to_delete != 0)
				{
					RBT_delete(&_base_tree, to_delete);
					_alloc_pair.destroy(to_delete->get_ptr_data());
					_alloc_pair.deallocate(to_delete->get_ptr_data(), 1);
			...
			void swap (map& x)
			{
				allocator_type save_alloc_pair = _alloc_pair;
		...
		private :
			...
			void	recursive_destructor(node *base)
			{
				...
				_alloc_pair.destroy(base->get_ptr_data());
				_alloc_pair.deallocate(base->get_ptr_data(), 1);
			...
			void recursive_assignment(node *base, node **new_base, const_iterator &first, const_iterator &last)
			{
				...
				{
					_alloc_pair.destroy(base->get_ptr_data());
					_alloc_pair.construct(base->get_ptr_data(), *first);
					base->RBT_reset();
