...
namespace ft
{
	template <class value_type, class Key, class Compare, class Allocator>
	class RBT_node
	{
		public :
		typedef value_type data_type;

		RBT_node(value_type *new_pair) : _left(0), _right(0), _father(0), _color(true), data(new_pair) { }
		...
		RBT_node	*get_right() {return (_right);};
		RBT_node	*get_left() {return (_left);};
		RBT_node	*get_father() {return (_father);};

		friend RBT_node *RBT_find(RBT_node *base, Key to_search, Compare comp)
		{
			while (base !=  0)
			{
				if (!comp(base->data->first,to_search) && !comp(to_search, base->data->first))
					return base;
				if (comp(base->data->first, to_search))
					base = base->_right;
				else
					base = base->_left;
			}
		...
		friend void RBT_recolor(RBT_node **base, RBT_node *new_node)
		{
			...
			if (new_node->_color == false || new_node->_father->_color == false)
				return ;
			RBT_node *father = new_node->_father;
			RBT_node *uncle = father->_father;
			uncle = (uncle->_left == father) ? uncle->_right : uncle->_left;
			if (uncle == 0 || uncle->_color == false)
			{
				RBT_node *grandfather = father->_father;
		...
		friend void RBT_insert(RBT_node **base, RBT_node *new_node, Compare comp)
		{
			if (*base == 0)
			{
				*base = new_node;
			...
			while(true)
			{
				if (comp(temp->data->first, new_node->data->first))
				{
					if (temp->_right == 0)
					{
		...
		friend void RBT_insert_position(RBT_node **base, Key const &k, RBT_node *new_node, Compare comp)
		{
			if (*base == 0)
			{
				*base = new_node;
			...
			RBT_node *hint = RBT_find(*base, k, comp);

			if (comp(hint->data->first, new_node->data->first))
			{
				while(hint->_father && comp(hint->data->first, new_node->data->first))
					hint = hint->_father;
			}
			else
			{
				if (hint->_father != 0 && !comp(hint->_father->data->first, new_node->data->first))
		...
		friend void RBT_double_black(RBT_node **base, RBT_node *to_fix)
		{
			if (to_fix->_father == 0)
				return ;
			else if (to_fix->_father->_left == to_fix)
			{
				RBT_node *brother = to_fix->_father->_right; //brother == 0 imposible ?
			...
			{
				RBT_node *brother = to_fix->_father->_left; //brother == 0 imposible ?
				if (brother->_color == false)
				{
					if ((brother->_left == 0 || brother->_left->_color == false) && (brother->_right == 0 || brother->_right->_color == false))
					{
		...
		friend void RBT_delete(RBT_node **base, RBT_node *to_delete)
		{
			...
			{
				RBT_node *temp = to_delete->_left;
				while (temp->_right != 0)
					temp = temp->_right;
				RBT_node *left_node = to_delete->_left;
				RBT_node *right_node = to_delete->_right;
				RBT_node *father_node = to_delete->_father;
		...
		friend RBT_node *RBT_min(RBT_node *base)
		{
			if (base == 0)
				return (0);
		...
		friend RBT_node *RBT_max(RBT_node *base)
		{
			if (base == 0)
				return (0);
		...
		//TO DELETE
		friend void print_RBT(RBT_node *base, int n = 0)
		{
			int save = n;
			if (base == 0)
			{
			...
			else
			{
				...
				std::cout << base->data->first;
				std::cout << " father = ";
				if (base->_father != 0)
					std::cout << base->_father->data->first;
				else
		...
		friend	void	set_extremum_node(RBT_node *base, RBT_node *nodemin, RBT_node *nodemax)
		{
			RBT_node *temp_min = RBT_min(base);
			RBT_node *temp_max = RBT_max(base);
		...
		friend	void	delete_extremum_node(RBT_node *base)
		{
			RBT_node *temp_min = RBT_min(base);
			RBT_node *temp_max = RBT_max(base);
